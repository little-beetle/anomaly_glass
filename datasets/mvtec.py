...
class MVTecDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == DatasetSplit.TRAIN:
            # ✅ замість self.x[index] використовуємо image_path з data_to_iterate
            img = PIL.Image.open(image_path).convert("RGB")

            # --- безпечна логіка вибору аугментації ---
            if hasattr(self, "anomaly_source_paths") and len(self.anomaly_source_paths) > 0:
            ...
            if self.class_fg:
                ...
                if os.path.exists(fgmask_path):
                    ...
                    mask_fg = torch.ones_like(image[0])
            else:
                mask_fg = torch.ones_like(image[0])
            # --------------------------------

            # --- створення масок через Perlin noise ---
