...
class GLASS(torch.nn.Module):
    ...
    def _train_discriminator(self, input_data, cur_epoch, pbar, pbar_str1):
        ...
        r_t = torch.tensor([0.0]).to(self.device)
        r_g = torch.tensor([0.0]).to(self.device)
        r_f = torch.tensor([0.0]).to(self.device)
    ...
    def predict(self, test_dataloader):
        ...
        with tqdm.tqdm(test_dataloader, desc="Inferring...", leave=False, unit='batch') as data_iterator:
            for data in data_iterator:
                ...
                self.current_labels = np.array(data["is_anomaly"].numpy())  # <--- додати ось це
    ...
    def _predict(self, img):
        ...
        with torch.no_grad():
            ...
            # --- Save embeddings and labels for AdaBoost ---
            import numpy as np, os, glob
            os.makedirs("embeddings", exist_ok=True)
            existing = len(glob.glob("embeddings/features_batch_*.npy"))
            np.save(f"embeddings/features_batch_{existing:03d}.npy", patch_features.detach().cpu().numpy())

            # якщо є мітки (0/1)
            if hasattr(self, "current_labels") and self.current_labels is not None:
                np.save(f"embeddings/labels_batch_{existing:03d}.npy", self.current_labels)
            # -----------------------------------------------
